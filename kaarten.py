...
#
# ══════════════════════════════════════════════════════════════════════════════
# 9. INTERNATIONALE GEBRUIKERS (puntenkaart)
# ══════════════════════════════════════════════════════════════════════════════
print("Kaart 9: internationale gebruikers...")

# Laad adressen
df_addr = pd.read_parquet(BASE / 'addresses.parquet')
df_addr = df_addr[df_addr['model_type'] == 'user'].dropna(subset=['lat', 'long'])
df_addr['lat']  = pd.to_numeric(df_addr['lat'],  errors='coerce')
df_addr['long'] = pd.to_numeric(df_addr['long'], errors='coerce')
df_addr = df_addr.dropna(subset=['lat', 'long'])

# Koppel aan scores
df_scores = pd.read_parquet(BASE / 'users_met_scores.parquet')
df_scores = df_scores.drop_duplicates(subset='user_id', keep='first')
df_scores['heartrisk_num'] = pd.to_numeric(df_scores['rec_heartrisk_cat'], errors='coerce')

df_punten = df_addr.merge(
    df_scores[['user_id', 'rec_user_gender', 'heartrisk_num', 'rec_med_bmi_cat', 'rec_ls_stress_cat']],
    left_on='model_id', right_on='user_id', how='left'
)
df_punten['geslacht'] = pd.to_numeric(df_punten['rec_user_gender'], errors='coerce').map({0: 'Vrouw', 1: 'Man'})

# Maak kaart gecentreerd op Europa
m_int = folium.Map(
    location=[52.0, 10.0],
    zoom_start=5,
    tiles=KAART_STIJL['tiles'],
    max_zoom=10,
)

# Voeg punten toe
for _, row in df_punten.iterrows():
    try:
        lat  = float(row['lat'])
        lon  = float(row['long'])
        gsl  = row.get('geslacht', '') or ''

        # Sla gebruikers zonder geslacht over
        if gsl not in ('Man', 'Vrouw'):
            continue

        city = row.get('city', '') or ''
        land = row.get('country', '') or ''
        kleur = GENDER_COLORS.get(gsl, '#888888')
        tooltip = f"{city}, {land} | {gsl}"
...
# Legenda
m_int.get_root().html.add_child(folium.Element('''
<div style="position:fixed;bottom:30px;left:10px;z-index:1000;
     background:white;padding:8px 12px;border-radius:6px;
     box-shadow:2px 2px 6px rgba(0,0,0,0.3);font-family:sans-serif;">
    <b style="font-size:12px;display:block;margin-bottom:5px;">Geslacht</b>
    <div style="display:flex;align-items:center;margin-bottom:3px;">
        <div style="width:12px;height:12px;background:#1F6FBF;border-radius:50%;margin-right:6px;"></div>
        <span style="font-size:11px;">Man</span>
    </div>
    <div style="display:flex;align-items:center;">
        <div style="width:12px;height:12px;background:#E86FA8;border-radius:50%;margin-right:6px;"></div>
        <span style="font-size:11px;">Vrouw</span>
    </div>
</div>'''))

m_int.save(str(HTML_MAP / 'internationale_gebruikers_kaart.html'))
print("  Opgeslagen.")
...
# ══════════════════════════════════════════════════════════════════════════════
# 9. WERELDKAART MET PUNTEN (inclusief buitenland)
# ══════════════════════════════════════════════════════════════════════════════
print("Kaart 9: wereldkaart met gebruikerspunten...")


df_addr_wereld = pd.read_parquet(BASE / 'addresses.parquet')
df_scores_wereld = pd.read_parquet(BASE / 'users_met_scores.parquet')
df_scores_wereld = df_scores_wereld.drop_duplicates(subset='user_id', keep='first')

# Koppel adressen aan gebruikers
df_addr_user = df_addr_wereld[
    (df_addr_wereld['model_type'] == 'user') &
    df_addr_wereld['lat'].notna() &
    df_addr_wereld['long'].notna()
].copy()

df_gebruikers = df_scores_wereld[['user_id', 'rec_user_gender',
                                    'rec_heartrisk_cat', 'rec_med_bmi_cat',
                                    'rec_ls_stress_cat']].merge(
    df_addr_user[['model_id', 'lat', 'long', 'city', 'country']],
    left_on='user_id', right_on='model_id', how='inner'
)

print(f"  Gebruikers met coördinaten: {len(df_gebruikers)}")
print(f"  Landen: {df_gebruikers['country'].value_counts().head(5).to_dict()}")

# Maak kaart
m_wereld = folium.Map(location=[52.1, 5.3], zoom_start=6,
                       tiles=KAART_STIJL['tiles'])

# Cluster punten voor performance
from folium.plugins import MarkerCluster
cluster = MarkerCluster().add_to(m_wereld)

for _, row in df_gebruikers.iterrows():
    folium.CircleMarker(
        ...
        fill_color='#E87722',
        fill_opacity=0.7,
        tooltip=f"{row.get('city', 'Onbekend')}, {row.get('country', '')}",
    ).add_to(cluster)

# Titel
m_wereld.get_root().html.add_child(folium.Element(
    '<div style="position:fixed;top:10px;left:50%;transform:translateX(-50%);'
    'z-index:1000;background:white;padding:8px 14px;border-radius:6px;'
    'box-shadow:2px 2px 6px rgba(0,0,0,0.3);font-family:sans-serif;'
    'font-size:13px;font-weight:bold;">Gebruikerslocaties wereldwijd</div>'
))

m_wereld.save(str(HTML_MAP / 'wereldkaart_gebruikers.html'))
print("  Opgeslagen.")
